<?php

return [

    'posts' => [
        [
            'en' => [
                'slug' => 'how-to-use-personas-in-product-development',
                'tags' => [
                    'startup' => 'Startup',
                ],
            ],
            'hu' => [
                'slug' => 'perszonak-a-termekfejlesztesben',
                'tags' => [
                    'startup' => 'Startup',
                ]
            ],
        ],
        [
            'en' => [
                'slug' => 'how-to-measure-the-success-of-your-startup',
                'tags' => [
                    'startup' => 'Startup',
                ],
            ],
            'hu' => [
                'slug' => 'hogyan-merjuk-a-termekunk-sikeresseget',
                'tags' => [
                    'startup' => 'Startup',
                ]
            ],
        ],
        [
            'en' => [
                'slug' => '11-ways-to-build-an-mvp-pt2',
                'tags' => [
                    'startup' => 'Startup',
                ],
            ],
            'hu' => [
                'slug' => '11-mod-hogy-mvp-t-epitsunk-2-resz',
                'tags' => [
                    'startup' => 'Startup',
                ]
            ],
        ],
        [
            'en' => [
                'slug' => '11-ways-to-build-an-mvp-pt1',
                'tags' => [
                    'startup' => 'Startup',
                ],
            ],
            'hu' => [
                'slug' => '11-mod-hogy-mvp-t-epitsunk-1-resz',
                'tags' => [
                    'startup' => 'Startup',
                ]
            ],
        ],
        [
            'en' => [
                'slug' => 'what-are-hypotheses-in-product-development',
                'tags' => [
                    'startup' => 'Startup',
                ],
            ],
            'hu' => [
                'slug' => 'hipotezisek-a-termekfejlesztesben',
                'tags' => [
                    'startup' => 'Startup',
                ]
            ],
        ],
        [
            'en' => [
                'slug' => 'the-lean-product-process-pt4',
                'tags' => [
                    'startup' => 'Startup',
                ],
            ],
            'hu' => [
                'slug' => 'lean-termekfejlesztesi-folyamat-4-resz',
                'tags' => [
                    'startup' => 'Startup',
                ]
            ],
        ],
        [
            'en' => [
                'slug' => 'the-lean-product-process-pt3',
                'tags' => [
                    'startup' => 'Startup',
                ],
            ],
            'hu' => [
                'slug' => 'lean-termekfejlesztesi-folyamat-3-resz',
                'tags' => [
                    'startup' => 'Startup',
                ]
            ],
        ],
        [
            'en' => [
                'slug' => 'the-lean-product-process-pt2',
                'tags' => [
                    'startup' => 'Startup',
                ],
            ],
            'hu' => [
                'slug' => 'lean-termekfejlesztesi-folyamat-2-resz',
                'tags' => [
                    'startup' => 'Startup',
                ]
            ],
        ],
        [
            'en' => [
                'slug' => 'the-lean-product-process-pt1',
                'tags' => [
                    'startup' => 'Startup',
                ],
            ],
            'hu' => [
                'slug' => 'lean-termekfejlesztesi-folyamat-1-resz',
                'tags' => [
                    'startup' => 'Startup',
                ]
            ],
        ],
        [
            'en' => [
                'slug' => 'effective-home-office-with-kids',
                'tags' => [
                    'remote-work' => 'Remote Work',
                ],
            ],
            'hu' => [
                'slug' => 'hatekony-home-office-a-gyerekek-mellett',
                'tags' => [
                    'tavmunka' => 'Távmunka',
                ],
            ],
        ],
        [
            'en' => [
                'slug' => 'improve-how-you-code-by-using-a-rubber-duck',
                'tags' => [
                    'development' => 'Development',
                ],
            ],
            'hu' => [
                'slug' => 'irjunk-jobb-kodot-egy-gumikacsa-segitsegevel',
                'tags' => [
                    'fejlesztes' => 'Fejlesztés',
                ]
            ],
        ],
        [
            'en' => [
                'slug' => 'validation-in-product-development',
                'tags' => [
                    'startup' => 'Startup',
                ],
            ],
            'hu' => [
                'slug' => 'validacio-a-termekfejlesztesben',
                'tags' => [
                    'startup' => 'Startup',
                ],
            ],
        ],
        [
            'en' => [
                'slug' => 'how-does-digitalization-change-workplaces',
                'tags' => [
                    'digital-transformation' => 'Digital Transformation',
                ],
            ],
            'hu' => [
                'slug' => 'hogyan-valtoztatja-meg-a-digitalizacio-a-munkahelyeinket',
                'tags' => [
                    'digitalis-transzformacio' => 'Digitális transzformáció',
                ]
            ],
        ],
        [
            'en' => [
                'slug' => 'protect-your-software',
                'tags' => [
                    'explained' => 'Explained',
                ],
            ],
            'hu' => [
                'slug' => 'vedd-meg-a-szoftvered',
                'tags' => [
                    'elmagyarazva' => 'Elmagyarazva',
                ]
            ],
        ],
        [
            'en' => [
                'slug' => 'dont-try-to-build-the-perfect-product',
                'tags' => [
                    'startup' => 'Startup',
                ],
            ],
            'hu' => [
                'slug' => 'ne-probalj-tokeletes-termeket-fejleszteni',
                'tags' => [
                    'startup' => 'Startup',
                ],
            ],
        ],
        [
            'en' => [
                'slug' => 'why-you-cant-afford-to-delay-digital-transformation',
                'tags' => [
                    'digital-transformation' => 'Digital Transformation',
                ],
            ],
            'hu' => [
                'slug' => 'miert-ne-halogasd-a-digitalis-transzformaciot',
                'tags' => [
                    'digitalis-transzformacio' => 'Digitális transzformáció',
                ],
            ],
        ],
        [
            'en' => [
                'slug' => 'technical-debt-how-to-build-future-proof-it-systems',
                'tags' => [
                    'explained' => 'Explained',
                ],
            ],
            'hu' => [
                'slug' => 'technikai-adossag-hogyan-legyenek-idotallo-it-rendszereink',
                'tags' => [
                    'elmagyarazva' => 'Elmagyarázva',
                ],
            ],
        ],
        [
            'en' => [
                'slug' => 'how-long-is-a-development-project',
                'tags' => [
                    'explained' => 'Explained',
                ],
            ],
            'hu' => [
                'slug' => 'milyen-hosszu-egy-fejlesztesi-projekt',
                'tags' => [
                    'elmagyarazva' => 'Elmagyarázva',
                ],
            ],
        ],
        [
            'en' => [
                'slug' => 'freelancer-vs-development-company-vs-build-your-own-team',
                'tags' => [
                    'explained' => 'Explained',
                ],
            ],
            'hu' => [
                'slug' => 'szabaduszokat-sajat-it-csapatot-vagy-kulso-fejleszto-partnert-valasszunk',
                'tags' => [
                    'elmagyarazva' => 'Elmagyarázva',
                ],
            ],
        ],
        [
            'en' => [
                'slug' => 'long-distance-relationship-in-a-corporate-environment',
                'tags' => [
                    'remote-work' => 'Remote Work',
                ],
            ],
            'hu' => [
                'slug' => 'mukodhet-e-sikeres-tavkapcsolat-ceges-kornyezetben',
                'tags' => [
                    'tavmunka' => 'Távmunka',
                ],
            ],
        ],
        [
            'en' => [
                'slug' => 'how-to-stay-motivated-when-you-work-remotely',
                'tags' => [
                    'remote-work' => 'Remote Work',
                ],
            ],
            'hu' => [
                'slug' => 'hogyan-motivaljuk-magunkat-ha-tavmunkaban-dolgozunk',
                'tags' => [
                    'tavmunka' => 'Távmunka',
                ],
            ],
        ],
        [
            'en' => [
                'slug' => 'how-to-build-a-remote-company-culture',
                'tags' => [
                    'remote-work' => 'Remote Work',
                ],
            ],
            'hu' => [
                'slug' => 'hogyan-epitsunk-remote-vallalati-kulturat',
                'tags' => [
                    'tavmunka' => 'Távmunka',
                ],
            ],
        ],
        [
            'en' => [
                'slug' => 'how-to-hire-remote-talent',
                'tags' => [
                    'remote-work' => 'Remote Work',
                ],
            ],
            'hu' => [
                'slug' => 'hogyan-vegyunk-fel-uj-remote-csapattagot',
                'tags' => [
                    'tavmunka' => 'Távmunka',
                ],
            ],
        ],
        [
            'en' => [
                'slug' => 'what-is-a-remote-team-and-is-it-for-you',
                'tags' => [
                    'remote-work' => 'Remote Work',
                ],
            ],
            'hu' => [
                'slug' => 'mi-az-a-remote-csapat-es-miert-szeretjuk-a-furthernel',
                'tags' => [
                    'tavmunka' => 'Távmunka',
                ],
            ],
        ],
        [
            'en' => [
                'slug' => '5-things-every-business-owner-should-know',
                'tags' => [
                    'explained' => 'Explained',
                ],
            ],
            'hu' => [
                'slug' => '5-hasznos-adat-cegvezetoknek',
                'tags' => [
                    'elmagyarazva' => 'Elmagyarázva',
                ],
            ],
        ],
        [
            'en' => [
                'slug' => 'how-does-a-software-optimise-your-manufacturing-processes',
                'tags' => [
                    'explained' => 'Explained',
                    'digital-transformation' => 'Digital Transformation',
                ],
            ],
            'hu' => [
                'slug' => 'hogyan-segit-egy-szoftver-a-termelesi-folyamatok-optimalizalasaban',
                'tags' => [
                    'elmagyarazva' => 'Elmagyarázva',
                    'digitalis-transzformacio' => 'Digitális transzformáció',
                ],
            ],
        ],
        [
            'en' => [
                'slug' => 'going-paperless-how-to-digitise-your-business',
                'tags' => [
                    'explained' => 'Explained',
                ],
            ],
            'hu' => [
                'slug' => 'hogyan-alakitsuk-at-cegunket-papirmentesse',
                'tags' => [
                    'elmagyarazva' => 'Elmagyarázva',
                ],
            ],
        ],
        [
            'en' => [
                'slug' => 'how-to-choose-the-right-crm',
                'tags' => [
                    'explained' => 'Explained',
                ],
            ],
            'hu' => [
                'slug' => 'hogyan-valasszuk-ki-a-megfelelo-crm-rendszert',
                'tags' => [
                    'elmagyarazva' => 'Elmagyarázva',
                ],
            ],
        ],
        [
            'en' => [
                'slug' => 'when-is-it-time-to-use-a-crm',
                'tags' => [
                    'explained' => 'Explained',
                ],
            ],
            'hu' => [
                'slug' => 'mikor-hasznaljunk-crm-rendszert',
                'tags' => [
                    'elmagyarazva' => 'Elmagyarázva',
                ],
            ],
        ],
        [
            'en' => [
                'slug' => 'digital-transformation-pt4-it',
                'tags' => [
                    'digital-transformation' => 'Digital Transformation',
                ],
            ],
            'hu' => [
                'slug' => 'digitalis-transzformacio-4-resz-technologia',
                'tags' => [
                    'digitalis-transzformacio' => 'Digitális transzformáció',
                ],
            ],
        ],
        [
            'en' => [
                'slug' => 'digital-transformation-pt3-business-processes',
                'tags' => [
                    'digital-transformation' => 'Digital Transformation',
                ],
            ],
            'hu' => [
                'slug' => 'digitalis-transzformacio-3-resz-uzleti-folyamatok',
                'tags' => [
                    'digitalis-transzformacio' => 'Digitális transzformáció',
                ],
            ],
        ],
        [
            'en' => [
                'slug' => 'digital-transformation-pt2-company-culture',
                'tags' => [
                    'digital-transformation' => 'Digital Transformation',
                ],
            ],
            'hu' => [
                'slug' => 'digitalis-transzformacio-2-resz-vallalati-kultura',
                'tags' => [
                    'digitalis-transzformacio' => 'Digitális transzformáció',
                ],
            ],
        ],
        [
            'en' => [
                'slug' => 'digital-transformation-pt1-introduction',
                'tags' => [
                    'digital-transformation' => 'Digital Transformation',
                ],
            ],
            'hu' => [
                'slug' => 'digitalis-transzformacio-1-resz-bevezeto',
                'tags' => [
                    'digitalis-transzformacio' => 'Digitális transzformáció',
                ],
            ],
        ],
        [
            'en' => [
                'slug' => 'laraconeu-madrid-2019',
                'tags' => [
                    'life-at-further' => 'Life at Further',
                ],
            ],
            'hu' => [
                'slug' => 'laraconeu-madrid-2019',
                'tags' => [
                    'elet-a-furthernel' => 'Élet a Furthernél',
                ],
            ],
        ],
        [
            'en' => [
                'slug' => 'the-story-behind-our-rebranding',
                'tags' => [
                    'life-at-further' => 'Life at Further',
                ],
            ],
            'hu' => [
                'slug' => 'uj-kontosben-koszontjuk-a-nyarat',
                'tags' => [
                    'elet-a-furthernel' => 'Élet a Furthernél',
                ],
            ],
        ],
        [
            'en' => [
                'slug' => 'what-makes-a-good-agile-software-specification',
                'tags' => [
                    'agile' => 'Agile',
                ],
            ],
            'hu' => [
                'slug' => 'mitol-lesz-jo-egy-agilis-szoftverspecifikacio',
                'tags' => [
                    'agile' => 'Agile',
                ],
            ],
        ],
        [
            'en' => [
                'slug' => 'custom-development-vs-out-of-the-box-software',
                'tags' => [
                    'explained' => 'Explained',
                ],
            ],
            'hu' => [
                'slug' => 'egyedi-fejlesztes-vagy-dobozos-szoftver',
                'tags' => [
                    'elmagyarazva' => 'Elmagyarázva',
                ],
            ],
        ],

    ],

    'tags' => [
        [
            'en' => [
                'description' => 'Our approach to agile, by our project management team.',
                'name' => 'Agile',
                'slug' => 'agile',
            ],
            'hu' => [
                'description' => 'Agile módszerek, ahogy a projektmenedzsment csapatunk látja.',
                'name' => 'Agile',
                'slug' => 'agile',
            ],
        ],
        [
            'en' => [
                'description' => 'Bits and thoughts, penned by our development team.',
                'name' => 'Development',
                'slug' => 'development',
            ],
            'hu' => [
                'description' => 'Bitek és gondolatok, a fejlesztőcsapatunk tollából.',
                'name' => 'Fejlesztés',
                'slug' => 'fejlesztes',
            ],
        ],
        [
            'en' => [
                'description' => 'Our thoughts on business, innovation and emerging technologies.',
                'name' => 'Digital Transformation',
                'slug' => 'digital-transformation',
            ],
            'hu' => [
                'description' => 'Gondolataink üzletről, innovációról és új technológiákról.',
                'name' => 'Digitális transzformáció',
                'slug' => 'digitalis-transzformacio',
            ],
        ],
        [
            'en' => [
                'description' => 'Frequently asked questions and light topics, clearly explained.',
                'name' => 'Explained',
                'slug' => 'explained',
            ],
            'hu' => [
                'description' => 'Ügyfeleinktől érkezett kérdések és könnyed témák, érthetően elmagyarázva.',
                'name' => 'Elmagyarázva',
                'slug' => 'elmagyarazva',
            ],
        ],
        [
            'en' => [
                'description' => 'Insights into our lives at Further.',
                'name' => 'Life at Further',
                'slug' => 'life-at-further',
            ],
            'hu' => [
                'description' => 'Betekintések a Further mindennapjaiba.',
                'name' => 'Élet a Furthernél',
                'slug' => 'elet-a-furthernel',
            ],
        ],
        [
            'en' => [
                'description' => 'Everything we learned in ' . (date('Y') - 2013) .' years of working remotely.',
                'name' => 'Remote Work',
                'slug' => 'remote-work',
            ],
            'hu' => [
                'description' => 'Minden amit ' . (date('Y') - 2013) .' év távmunka alatt megtanulunk.',
                'name' => 'Távmunka',
                'slug' => 'tavmunka',
            ],
        ],
        [
            'en' => [
                'description' => 'What we learned as development partners on over a dozen startup projects.',
                'name' => 'Startup',
                'slug' => 'startup',
            ],
            'hu' => [
                'description' => 'Amit fejlesztőpartnerként megtanultunk több mint egy tucat startup projekt alatt.',
                'name' => 'Startup',
                'slug' => 'startup',
            ],
        ],
        [
            'en' => [
                'description' => 'How UX research and design impacts our work as a development agency?',
                'name' => 'UX',
                'slug' => 'ux',
            ],
            'hu' => [
                'description' => 'Hogyan hat a UX kutatás és tervezés egy fejlesztő ügynökség munkájára?',
                'name' => 'UX',
                'slug' => 'ux',
            ],
        ],
    ],
];
